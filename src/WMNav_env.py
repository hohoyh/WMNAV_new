...
class Env:
    ...
    def _check_goal_visible(self, agent_position, goal_category, threshold=1.0):
        ...
        """✅ 修复：兼容单目标(List)和多目标(Dict)的数据结构"""
        if self.current_episode.get('is_multi_object', False):
            goal_positions = self.current_episode.get('object_positions', {}).get(goal_category, [])
        else:
            goal_positions = self.current_episode.get('object_positions', [])
        
        # 🔴 核心修复：针对大件家具，动态放大成功判定距离！
        # 如果你不想写死类别，也可以直接全局把阈值改为 1.5 或 2.0
        dynamic_threshold = self.cfg['success_threshold']
        if goal_category in ['bed', 'sofa', 'couch', 'dining table', 'bathtub']:
            dynamic_threshold = 2.0  # 大物体，只要靠近到 2 米以内就算找到！
        elif goal_category in ['tv screen', 'toilet', 'refrigerator']:
            dynamic_threshold = 1.5  # 中等物体
...
class WMNavEnv(Env):
    ...
    def _initialize_episode(self, episode_ndx: int):
        ...
        # ✅ Step 2: 标准化物体名称
        if episode.get('object_category') == 'tv_monitor':
        ...
        if self.is_multi_object:
            # === 多目标模式 ===
            # 读取类别列表
            object_categories = episode.get('object_categories', [episode.get('object_category')])
            ...
            if all_view_positions:
                ...
                logging.info(f"Using {len(all_view_positions)} view positions for path planning")
            else:
                # 如果找不到观察点，使用虚拟目标
                ...
                logging.warning("No view positions found - using exploration mode")
            
        else:
            # === 单目标模式 ===
            # 只有在单目标模式下才去查 self.goals
            goals = self.goals[f[1][6:]]
            # 🔴 修复：单目标模式下，如果是电视，去底层查的时候也必须翻译回真名！
            raw_cat = 'tv_monitor' if episode["object_category"] == 'tv screen' else episode["object_category"]
            obj_key = f'{scene_id_key}_{raw_cat}'
            # all_objects = goals[f'{scene_id_key}_{episode["object_category"]}']
            ...
            # print(f"[DEBUG] Single-Object Episode: {episode['object_category']}")
            ...
            if obj_key in goals:
                ...
                self.current_episode = {
                    'object': episode['object_category'], # 这里依旧保持 'tv screen' 给大模型看
                    'shortest_path': episode['info']['geodesic_distance'],
                    'object_positions': [obj['position'] for obj in all_objects],
                    'view_positions': view_positions,
                    'is_multi_object': False,
    ...
    def _step_env(self, obs: dict):
            ...
            if display_goal is not None:
                ...
                text_thickness = 2
                (text_width, text_height), _ = cv2.getTextSize(f"goal:{display_goal}", cv2.FONT_HERSHEY_SIMPLEX, text_size, text_thickness)
                text_position = (color_origin.shape[1] - text_width - padding, padding + text_height)
                cv2.putText(color_origin, f"goal:{display_goal}", text_position, cv2.FONT_HERSHEY_SIMPLEX, text_size, (255, 0, 0), text_thickness, cv2.LINE_AA)
