...
class VLMNavAgent(Agent):
    ...
    def step(self, obs: dict, goal_num: dict=None):
        ...
        if len(self.position_history) >= 5:
            ...
            # 🚨 触发条件：提高警觉度！移动小于 0.6 米或在同一个地方绕圈 4 次，直接判定危险
            is_stuck = (avg_movement < 0.6) or (nearby_count > 4)
            
            # 🔴 极速卡死检测：如果连续两步加起来都没走够 0.1 米（说明在原地转圈或贴脸撞墙），立刻触发！
            if len(distances) >= 2 and sum(distances[-2:]) < 0.1:
                is_stuck = True
            ...
            if is_stuck:
                ...
                if self.stuck_counter >= 2: 
                    ...
                    # 🔴 核心修复 2：上帝之手必须强行选择“物理距离最长”的路线逃生！
                    a_final = metadata['a_final'] # 这里也是字典
                    if a_final:
                        # 从字典中找出物理距离 r (item[0][0]) 最大的项
                        longest_item = max(a_final.items(), key=lambda item: item[0][0])
                        forced_action_number = longest_item[1]
                        
                        metadata['step_metadata']['action_number'] = forced_action_number
                        # 注意：字典取键需要加上 .keys() 转换为列表
                        agent_action = self._action_number_to_polar(forced_action_number, list(a_final.keys()))
                        self.turned = self.step_ndx - self.cfg['turn_around_cooldown']
                        logging.warning(f"🏃 GOD MODE Forced Escape: Chose longest action {forced_action_number} (distance: {longest_item[0][0]:.2f}m)")
                        
                    # ❌ 注意：旧版本的 valid_indices 和 random.choice 相关代码已经被彻底删除了！
            else:
...
class WMNavAgent(VLMNavAgent):
    ...
    def _initialize_vlms(self, cfg: dict):
            ...
            self.ActionVLM = api.GeminiVLM(model="gpt-4o-mini", system_instruction=action_system_instruction)
            self.PlanVLM = api.GeminiVLM(model="gpt-4o-mini", system_instruction=action_system_instruction)
            self.GoalVLM = api.GeminiVLM(model="gpt-4o-mini", system_instruction=action_system_instruction)
    ...
    def _choose_action(self, obs: dict, goal_num: dict=None):
        ...
        if step_metadata['called_stopping']:
            ...
            # 🔴 核心修复：严禁局部小脑临阵脱逃！
            if step_metadata.get('action_number') == 0 and len(a_final) > 0:
                logging.warning("🚨 Anti-Cowardice Triggered: Action VLM chose 0. Forcing forward movement!")
                # a_final 此时是一个字典 {(r, theta): action_number}
                # 取出物理距离 r 最大（最长）的那个动作
                longest_item = max(a_final.items(), key=lambda item: item[0][0])
                step_metadata['action_number'] = longest_item[1]
            ...
            try:
                ...
                for angle, values in target_dict.items():
                    ...
                    if isinstance(values, dict):
                        ...
                        # 🔴 提取所有目标分数并转为小写，以便后续精准匹配
                        angle_scores = {}
                        for k, v in values.items():
                            if k.lower() != 'explanation':
                                try:
                                    angle_scores[k.lower()] = float(v)
                                except (ValueError, TypeError):
                                    pass
                        
                        # 兜底：如果大模型依然只输出了一个统称的 'score'
                        if 'score' in angle_scores and len(angle_scores) == 1:
                            explorable_value[angle] = angle_scores['score']
                        else:
                            explorable_value[angle] = angle_scores
                    else:
                        explorable_value[angle] = 0
                        reason[angle] = ""
            except Exception as e:
                ...
                for i in range(12):
                    ...
                    if angle in explorable_value:
                        ...
                        if isinstance(val, (int, float)):
                            ...
                            # 🔴 精准提取：用小写的 target_goal 去匹配
                            score = float(val.get(target_goal.lower(), val.get('score', 0)))
                    # 更新地图 (Vectorized操作)
            ...
            for i in range(12):
                ...
                if angle in explorable_value:
                    ...
                    if isinstance(val, (int, float)):
                        ...
                        keys_upper = {str(k).upper(): v for k, v in val.items()}
                        target_upper = str(current_target).upper()
                        if target_upper in keys_upper:
                            vlm_score = float(keys_upper[target_upper])
                        else:
                            vlm_score = float(keys_upper.get('SCORE', 0.0))

                # 2. 获取地图记忆分数（采纳 Claude 的 75 分位数，完美过滤噪点）
                if angle in self.panoramic_mask and not np.all(self.panoramic_mask[angle] == False):
                    # 如果有至少 25% 的未探索区域，就会拿到高分；否则直接压死
                    map_score = np.percentile(fused_map[self.panoramic_mask[angle]], 75)
                else:
                    map_score = 10.0  

                # 3. 动态计算最终分数 (支持长距离跨越灰色区域)
                if vlm_score >= 9.5:
                    # 绝对视觉锁定：无视一切地图惩罚，直接冲
                    final_score[i] = vlm_score
                elif vlm_score >= 5.0:
                    # 🔴 核心修复 A：跨越灰色大厅的特权！
                    # VLM 觉得远方有希望（比如看到了另一扇门），哪怕脚下全是灰的（map_score 为 0），
                    # 也必须保留 40% 的 VLM 分数作为保底，让它可以大步跨越灰色区域！
                    memory_factor = map_score / 10.0
                    base_score = vlm_score * 0.4 
                    final_score[i] = max(vlm_score * memory_factor, base_score)
                else:
                    # VLM 觉得没啥希望（0~4分），此时如果还是灰色区域，狠狠惩罚（二次方），直接打死
                    memory_factor = (map_score / 10.0) ** 2
                    final_score[i] = vlm_score * memory_factor
                    
                # 打印测谎仪日志
                logging.info(f"[DEBUG-SCORES] Angle {angle}: VLM={vlm_score:.1f}, MapScore={map_score:.2f} -> Final={final_score.get(i, 0):.2f}")

            logging.info(f"Curiosity scores: { {str(int(i*30)): round(final_score[i],1) for i in final_score} }")
            
            # # 🔴 核心机制：物理探索动量 (Forward Momentum)
            # # 鼓励 Agent 保持向前探索，减少在开阔区域无意义的左右转头
            # # 330度和30度代表正前方，150度和210度代表向后
            # for i in final_score:
            #     if i in [1, 11]:  # 30度 和 330度
            #         final_score[i] *= 1.25  # 向前奖励 25%
            #     elif i in [5, 7]:   # 150度 和 210度
            #         final_score[i] *= 0.8   # 向后惩罚 20%
            
           # 4. 🔴 唤醒终极价值荒漠逃生机制 (Value Desert)
            if not final_score or max(final_score.values()) < 1.0:
                logging.warning("⚠️ Value Desert! All scores are too low (<1.0). Picking the most physically open green direction.")
                best_i = 1
                max_green = -1
                
                for i in range(1, 12, 2):
                    angle = str(int(i * 30))
                    if angle in self.effective_mask:
                        green_pixels = np.sum(self.effective_mask[angle])
                        
                        # 🔴 核心修复 B：防溜溜球机制 (物理惯性)
                        # 150度和210度代表向后转。狠狠惩罚向后的方向，强迫 Agent 保持向前的探索惯性！
                        if angle in ['150', '210']:
                            green_pixels *= 0.1 
                        # 30度和330度代表正前方。给予奖励，鼓励它穿透眼前的灰色区域！
                        elif angle in ['30', '330']:
                            green_pixels *= 1.5
                            
                        if green_pixels > max_green:
                            max_green = green_pixels
                            best_i = i
                idx = best_i
            else:
            ...
            if final_score:
                max_vlm_score = max([
                    float({str(k).upper(): v for k, v in explorable_value[str(int(i*30))].items()}.get(str(current_target).upper(), 0))
                    if isinstance(explorable_value.get(str(int(i*30))), dict) 
                    else float(explorable_value.get(str(int(i*30)), 0))
                    for i in range(1, 12, 2) if str(int(i*30)) in explorable_value
                ] + [0.0])
            
            self._last_curiosity_max_score = float(max_vlm_score)
            ...
            if angle_str in reason:
                ...
                if isinstance(r_val, str): final_reason = r_val
                    ...
                    keys_upper = {str(k).upper(): v for k, v in r_val.items()}
                    target_upper = str(current_target).upper()
                    final_reason = keys_upper.get(target_upper, keys_upper.get('EXPLANATION', ""))
    ...
    def make_plan(self, pano_images, previous_subtask, goal_reason, goal, goal_num):
        ...
        # 如果GPT-4o给了满分（10分），直接强制goal_flag=True
        # 因为Qwen-3B有时看不出全景图里的目标，但GPT-4o的打分更可靠
        if not goal_flag and hasattr(self, '_last_curiosity_max_score') and self._last_curiosity_max_score >= 10.0:
            logging.info(f"GPT-4o gave max score {self._last_curiosity_max_score}, forcing goal_flag=True")
            goal_flag = True

        # 保存本步goal_flag供下一步update_curiosity_value使用
        self._prev_goal_flag_agent = goal_flag
        logging.info(f"PLANNING_RESPONSE: goal_flag={goal_flag}, subtask={subtask}")
...
#             return evaluator_prompt
        ...
        if prompt_type == 'predicting':
            ...
            # 生成类似 '"SOFA": <0-10>, "BED": <0-10>' 的严格格式
            score_format = ", ".join([f"\"{t.upper()}\": <0-10>" for t in target_list])

            target_hints = []
            for t in target_list:
                t_low = t.lower().replace('_', ' ')
                if t_low == 'chair': target_hints.append("CHAIR: has backrest/seat")
                elif t_low == 'toilet': target_hints.append("TOILET: in bathrooms")
                elif t_low == 'bed': target_hints.append("BED: in bedrooms")
                elif t_low in ('tv screen', 'tv monitor', 'tv_monitor'): target_hints.append("TV/MONITOR: flat screen")
                elif t_low in ('sofa', 'couch'): target_hints.append("SOFA: multi-seat couch")
                elif t_low == 'plant': target_hints.append("PLANT: potted indoor plant")
                else: target_hints.append(f"{t.upper()}: look carefully")
            hints_str = " | ".join(target_hints)

            evaluator_prompt = (
                f"{progress_context}\n"
                f"You are a robot searching for THESE targets concurrently: {targets_str}.\n"
                f"Target guide: {hints_str}\n"
                f"The panoramic image has 6 panels labeled by angle: 30, 90, 150, 210, 270, 330.\n"
                f"\n"
                f"SCORING RULES (Evaluate EACH target INDEPENDENTLY for EACH angle):\n"
                f"  10 = This specific target is DIRECTLY VISIBLE.\n"
                f"  7-9 = The typical room for this target is visible.\n"
                f"  5-6 = Open door/hallway that COULD lead to this target.\n"
                f"  0-4 = Dead end, or unlikely to find this target here.\n"
                f"\n"
                f"Output ONLY valid JSON. Format EXACTLY like this:\n"
                f"{{\n"
                f"  \"scores\": {{\n"
                f"    \"30\": {{\"Explanation\": \"<what you see>\", {score_format}}},\n"
                f"    \"90\": {{\"Explanation\": \"<what you see>\", {score_format}}},\n"
                f"    \"150\": {{\"Explanation\": \"<what you see>\", {score_format}}},\n"
                f"    \"210\": {{\"Explanation\": \"<what you see>\", {score_format}}},\n"
                f"    \"270\": {{\"Explanation\": \"<what you see>\", {score_format}}},\n"
                f"    \"330\": {{\"Explanation\": \"<what you see>\", {score_format}}}\n"
                f"  }},\n"
                f"  \"action\": 0\n"
                f"}}"
            )
        ...
        # # ==================== 2. Predicting 提示词 (注入语义场景图先验) ====================
        # if prompt_type == 'predicting':
        #     # 🔴 修复此前大模型把列表字符串当成单个目标名的 Bug
        #     target_list = []
        #     if goal_num and 'remaining' in goal_num and goal_num['remaining']:
        #         raw_targets = goal_num['remaining']
        #         if len(raw_targets) == 1 and isinstance(raw_targets[0], str) and '[' in raw_targets[0]:
        #             import ast
        #             try: target_list = ast.literal_eval(raw_targets[0])
        #             except: target_list = [goal_str]
        #         else:
        ...
        #     targets_str = ", ".join([t.upper() for t in target_list])
        #     goal_lower = ", ".join([t.lower() for t in target_list])

        #     target_hints = []
        #     for t in target_list:
        #         t_low = t.lower().replace('_', ' ')
        #         if t_low == 'chair': target_hints.append("CHAIR: Living room, Dining room, Office")
        #         elif t_low == 'toilet': target_hints.append("TOILET: Bathroom ONLY")
        #         elif t_low == 'bed': target_hints.append("BED: Bedroom ONLY")
        #         elif t_low in ('tv screen', 'tv monitor', 'tv_monitor'): target_hints.append("TV/MONITOR: Living room, Bedroom")
        #         elif t_low in ('sofa', 'couch'): target_hints.append("SOFA: Living room ONLY")
        #         else: target_hints.append(f"{t.upper()}: General areas")
        #     hints_str = " | ".join(target_hints)

        #     # 动态生成 JSON 模板，确保每个目标独立打分
        #     score_format = ", ".join([f"\"{t.upper()}\": <0-10>" for t in target_list])

        #     evaluator_prompt = (
        #         f"{progress_context}\n"
        #         f"You are a strict logical robot searching for: {targets_str}.\n"
        #         f"Typical Room Guide: {hints_str}\n"
        #         f"\n"
        #         f"The panoramic image has 6 panels labeled by angle: 30, 90, 150, 210, 270, 330.\n"
        #         f"\n"
        #         f"🚨 STRICT SCORING RULES (Evaluate EACH target independently):\n"
        #         f"  10 = Target object is DIRECTLY VISIBLE.\n"
        #         f"  7-9 = Target's TYPICAL ROOM is visible (e.g., Bedroom for Bed).\n"
        #         f"  5-6 = Hallway or Stairs leading to UNSEEN areas.\n"
        #         f"  0-2 = 🚫 INCOMPATIBLE ROOMS. If searching for a BED or TOILET, a Kitchen/Dining Room MUST score 0-2! Do NOT assign high scores to incompatible rooms just because they are open spaces.\n"
        #         f"  0 = Dead end, blank wall, or closed door.\n"
        #         f"\n"
        #         f"Output ONLY valid JSON:\n"
        #         f"{{\n"
        #         f"  \"scores\": {{\n"
        #         f"    \"30\": {{\"Explanation\": \"<what room/space you see>\", {score_format}}},\n"
        #         f"    \"90\": {{\"Explanation\": \"<what room/space you see>\", {score_format}}},\n"
        #         f"    \"150\": {{\"Explanation\": \"<what room/space you see>\", {score_format}}},\n"
        #         f"    \"210\": {{\"Explanation\": \"<what room/space you see>\", {score_format}}},\n"
        #         f"    \"270\": {{\"Explanation\": \"<what room/space you see>\", {score_format}}},\n"
        #         f"    \"330\": {{\"Explanation\": \"<what room/space you see>\", {score_format}}}\n"
        #         f"  }},\n"
        #         f"  \"action\": 0\n"
        #         f"}}"
        #     )
        ...
        #     # 每个目标单独列出，让模型判断"任意一个可见就True"
        #     target_lines = []
        #     for t in target_list:
        #         t_low = t.lower().replace('_', ' ')
        #         if t_low == 'chair':
        #             target_lines.append("- CHAIR: has a visible backrest and seat (NOT a stool, bench, or sofa)")
        #         elif t_low == 'toilet':
        #             target_lines.append("- TOILET: white ceramic bowl with tank, found in bathrooms")
        #         elif t_low == 'bed':
        #             target_lines.append("- BED: mattress with headboard or bedding, found in bedrooms")
        #         elif t_low in ('tv screen', 'tv monitor', 'tv_monitor'):
        #             target_lines.append("- TV/MONITOR: flat screen display mounted on wall or on a stand")
        #         elif t_low == 'sofa':
        #             target_lines.append("- SOFA: large upholstered multi-seat couch (NOT a single chair)")
        #         elif t_low == 'plant':
        #             target_lines.append("- PLANT: potted plant or indoor plant with visible leaves/stems")
        #         else:
        #             target_lines.append(f"- {t.upper()}: look carefully for this object in the image")
        #     targets_desc = "\n".join(target_lines)
        #     targets_str = ", ".join([t.upper() for t in target_list])

        #     planning_prompt = (
        #         f"You are a robot navigator. Look at this image carefully.\n"
        #         f"You are searching for ANY ONE of these targets:\n"
        #         f"{targets_desc}\n"
        #         f"\n"
        #         f"STRICT RULE for Flag:\n"
        #         f"Flag=True ONLY IF you can directly see the target object in this image (even partially).\n"
        #         f"Flag=False if the target is NOT visible — even if you think it might be nearby or in a connected room.\n"
        #         f"'Possible', 'potential', 'could contain' → Flag=False.\n"
        #         f"You CANNOT go through closed doors or up/down stairs.\n"
        #         f"\n"
        #         f"OUTPUT ONLY JSON (no other text):\n"
        #         f"{{\"Subtask\": \"<where to go next>\", \"Flag\": <True or False>}}"
        #     )
        #     return planning_prompt
        # ==================== 3. Planning 提示词 (GPT 聪明+防呆版) ====================
        if prompt_type == 'planning':
            if goal_num and 'remaining' in goal_num and goal_num['remaining']:
                target_list = goal_num['remaining']
            else:
            ...
            targets_desc = ", ".join([t.upper() for t in target_list])

            planning_prompt = (
                f"You are a robot navigator. Look at this image carefully.\n"
                f"You are searching for ANY ONE of these targets: {targets_desc}\n"
                f"The AI brain evaluated this general direction and gave the reasoning: '{reason}'.\n"
                f"\n"
                f"Step 1: Determine your next Subtask based on the visual observation. "
                f"CRITICAL RULE: You MUST output a MOVEMENT action (e.g., 'Walk towards the toilet', 'Enter the bedroom'). "
                f"NEVER output 'Stay in the current location' or 'Wait'. The robot MUST keep moving!\n"
                f"Step 2: Check if ANY part of the target objects ({targets_desc}) is DIRECTLY VISIBLE in this image. "
                f"If you can physically see the target, set Flag to True. If it is NOT visible, set Flag to False.\n"
                f"You CANNOT go through closed doors or up/down stairs.\n"
                f"\n"
                f"OUTPUT ONLY JSON (no other text):\n"
                f"{{\n"
                f"  \"Subtask\": \"<where to go next>\",\n"
